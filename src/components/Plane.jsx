...
//   ({ onFire, joystick = { x: 0, y: 0 }, controls = {} }, ref) => {
//     const group = useRef();
//     const propellerRef = useRef();
//     const keys = useRef({});
//     const { scene } = useGLTF("/models/plane.glb");
...
//       if (isControlActive("arrowleft"))
//         plane.rotation.y +=
//           joystick.x > 0.3 ? 0.3 : joystick.x || ROTATION_SPEED;
//       if (isControlActive("arrowright"))
//         plane.rotation.y -=
//           -joystick.x > 0.3 ? 0.3 : -joystick.x || ROTATION_SPEED;
...
const Plane = forwardRef(
  ({ onFire, joystick = { x: 0, y: 0, distance: 0 }, controls = {} }, ref) => {
    const group = useRef();
    const propellerRef = useRef();
    const keys = useRef({});
    const { scene } = useGLTF("/models/plane.glb");
    ...
    useFrame((state) => {
      ...
      const { x, y, distance } = joystick;

      // === 📌 ROTATE plane ===
      plane.rotation.x =
        y !== 0
          ? THREE.MathUtils.clamp(
              plane.rotation.x + y * ROTATION_SPEED,
              -Math.PI / 4,
              Math.PI / 4
            )
          : 0;
      ...
      x !== 0
        ? (plane.rotation.y -= x * ROTATION_SPEED)
        : (plane.rotation.y = 0);

      // === 🚀 MOVE forward based on joystick distance ===
      const speed = (distance / 100) * MAX_SPEED;
      const forward = new THREE.Vector3();
      plane.getWorldDirection(forward);
      plane.position.addScaledVector(forward, -speed);
